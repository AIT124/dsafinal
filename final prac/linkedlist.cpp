...
void end_insert(node **head_ref , string namedata, string prndata){
   ...
   newnode->name = namedata;//the new node formed is storing name data in name block 
   newnode->prn = prndata;//the new node formed is storing prn data in prn block
   newnode->next = NULL;//putting the new node ka next equal to null
   node * temp = *head_ref;//pointer at head

   if(*head_ref==NULL){//when there is no node in linklist
       *head_ref = newnode;
       return ;
   }
   while(temp->next!=NULL){//jab tak linkedlist khatam nhi ho jati
       temp = ->next;
   }
   temp->next = newnode;//put the value of the end or the value we want to insert at node newnode
...
void deletion(node **head_ref, string target){
    ...
    if(temp->name==target){//to delete at head
        *head_ref = temp ->next;
        free(temp);
        return;
    }
